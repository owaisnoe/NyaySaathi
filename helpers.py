"""
helpers.py
Utility helpers for Nyay-Saathi Streamlit app.

This trimmed helpers.py focuses on minimizing unnecessary Gemini model instantiations
and providing robust small helpers used by the Streamlit app:
- get_cached_genai_model: cached genai.GenerativeModel factory (cached by model+temperature)
- retry_call: small retry wrapper
- JSON parsing helpers (cleaning and robust parsing of LLM JSON outputs)
- trim_to_chars / format_docs helpers for prompt trimming
- cached_retrieve: small wrapper to cache retriever calls (keeps original behavior)
- doc_hash: a short SHA256-based content hash utility

Note: This helpers file intentionally omits OCR-related functions (you decided to rely on the model for extraction).
"""
...
@st.cache_resource
def get_cached_genai_model(model_name: str, temperature: float = 0.2):
    """Return a cached genai.GenerativeModel instance keyed by (model_name, temperature).

    This avoids repeatedly constructing new model objects during the session.
    genai.configure(...) must be called before this.
    """
    if genai is None:
        raise RuntimeError("google.generativeai (genai) is not available in this environment.")
    # st.cache_resource will cache per distinct args (model_name, temperature)
    return genai.GenerativeModel(model_name, temperature=temperature)
